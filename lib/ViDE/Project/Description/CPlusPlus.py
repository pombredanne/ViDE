...
def __CppObject( source, additionalDefines, localLibraries, externalLibraries, explicit = False ):
    return Project.inProgress.createArtifact( Project.inProgress.context.buildkit.CPlusPlus.Object, source, additionalDefines, localLibraries, externalLibraries, explicit )
...
def CppDynamicLibrary( name, headers, sources = [], objects = [], localLibraries = [], externalLibraries = [], stripHeaders = identity ):
    ...
    binary = Project.inProgress.context.buildkit.Binary.DynamicLibraryBinary( Project.inProgress.context, name, __CppObjects( __CppSources( sources ), objects, [ "BUILD_" + name.upper() ], localLibraries, externalLibraries ), localLibraries, externalLibraries, False )
...
def CppStaticLibrary( name, headers, sources = [], objects = [], localLibraries = [], externalLibraries = [], stripHeaders = identity ):
    headers = __CppHeaders( headers )
    binary = Project.inProgress.context.buildkit.Binary.StaticLibraryBinary( Project.inProgress.context, name, __CppObjects( __CppSources( sources ), objects, [], localLibraries, externalLibraries ), localLibraries, externalLibraries, False )
